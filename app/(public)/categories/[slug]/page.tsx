  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-10">
      <div className="space-y-3">
        ...
        <h1 className="text-4xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">{category.name}</h1>
        <p className="max-w-3xl text-sm leading-7 text-slate-600 dark:text-slate-400">Browse our collection of {category.name} products.</p>
      </div>
      
      {products.length === 0 ? (
        <div className="mt-10 py-12 text-center text-slate-500 dark:text-slate-400">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => (
            <article key={product.id} className="overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm transition hover:shadow-lg">
              ...
              <div className="p-6">
                <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-50">{product.name}</h2>
