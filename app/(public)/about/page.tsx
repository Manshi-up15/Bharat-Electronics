    ...
    <main className="mx-auto max-w-7xl px-6 py-10">
      <div className="space-y-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-10 shadow-sm">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">{businessName}</h1>
        </div>
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="space-y-4">
            <p className="text-lg leading-8 text-slate-600 dark:text-slate-400">{aboutContent}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">Mission: Provide reliable and affordable electrical solutions.</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">Vision: Become the most trusted electronics store in the region.</p>
          </div>
